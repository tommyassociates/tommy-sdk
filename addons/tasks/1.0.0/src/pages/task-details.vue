<template>
	<f7-popup @popup:close="onPopupClose">
		<f7-view :init="false">
			<f7-page id="tasks__task" name="tasks__task" class="tasks__page">
				<f7-navbar noborder class="text-color-white">
					...
					<f7-nav-title text-color="white">
						{{ taskName }}
					</f7-nav-title>
					<f7-nav-right>
						<f7-link
							popover-open=".task-menu-popover"
							icon-f7="add"
						>
						</f7-link>
					</f7-nav-right>
				...
				<f7-popover class="task-menu-popover">
					<f7-list>
						<f7-list-button popover-close @click="addChecklist">
							{{ $t("tasks.task.checklist", "Checklist") }}
						</f7-list-button>
						<f7-list-button popover-close @click="addDeadline">
							{{ $t("tasks.task.deadline", "Deadline") }}
						</f7-list-button>
					</f7-list>
				</f7-popover>
				<f7-messagebar
					:placeholder="
						$t('tasks.task.enter_comment', 'Enter comment')
					"
					:value="comment"
				...
				<template v-if="task">
						...
						<input
							...
							:value="
								task.status === 'Unassigned'
									? $t(
											'tasks.task.waiting_for_assignments',
											'Waiting for assignments'
									  )
									: taskStatus(task.status)
							"
							readonly="readonly"
							class="unstyled task-status-picker"
						/>
					</f7-block>
					<participants-select
						:tags="task.filters || []"
						@tagtoggle="onTagToggle"
					>
					</participants-select>
					<f7-list media-list class="list-custom details bottom-0">
						<f7-list-item>
							<template #media>
								<img
									:src="`${$addonAssetsUrl}slice19.png`"
									:srcset="`${$addonAssetsUrl}slice19@2x.png 2x, ${$addonAssetsUrl}slice19.png 3x`"
								/>
							</template>
							<f7-input
								type="textarea"
					...
					<f7-list
						media-list
						class="list-custom details top-0 bottom-0"
						v-show="showDeadline || task.end_at"
					>
						<f7-list-item>
							<template #media>
								<img
									:src="`${$addonAssetsUrl}slice13.png`"
									:srcset="`${$addonAssetsUrl}slice13@2x.png 2x,${$addonAssetsUrl}slice13.png 3x`"
								/>
							</template>
							<template #inner>
								<div class="item-input-wrap">
									<input
										ref="deadlineInput"
										type="text"
										class="unstyled edit-task-deadline"
										:placeholder="
											$t(
												'tasks.task.edit_deadline',
												'Edit deadline...'
											)
										"
									/>
								...
								<a
									href="#"
									class="icon-only remove-deadline"
									@click="removeDeadline"
								>
									<img
										:src="`${$addonAssetsUrl}slice12.png`"
										:srcset="`${$addonAssetsUrl}slice12@2x.png 2x,${$addonAssetsUrl}slice12.png 3x`"
									/>
								</a>
							</template>
						</f7-list-item>
     ...
     -->
					<f7-list
						media-list
						class="list-custom checklist top-0"
						v-if="
							showChecklist ||
							(task.data.checklist && task.data.checklist.items)
						"
					>
						<li class="item-content">
							<div class="item-media">
								<img
									:src="`${$addonAssetsUrl}slice17.png`"
									:srcset="`${$addonAssetsUrl}slice17@2x.png 2x,${$addonAssetsUrl}slice17.png 3x`"
								/>
							</div>
							<div class="item-inner">
								...
								<a
									href="#"
									class="icon-only remove-checklist"
						...
						>
							<div class="item-media">
								<img
									v-if="!item.complete"
									:src="`${$addonAssetsUrl}slice14.png`"
									:srcset="`${$addonAssetsUrl}slice14@2x.png 2x,${$addonAssetsUrl}slice14.png 3x`"
								/>
								<img
									v-else
									:src="`${$addonAssetsUrl}slice15.png`"
									:srcset="`${$addonAssetsUrl}slice15@2x.png 2x,${$addonAssetsUrl}slice15.png 3x`"
								/>
							</div>
							<div class="item-inner">
								{{ item.text }}
								<a
									href="#"
									class="icon-only remove-checklist-item"
						...
						<li class="item-content">
							<div class="item-media">
								<img
									:src="`${$addonAssetsUrl}slice16.png`"
									:srcset="`${$addonAssetsUrl}slice16@2x.png 2x,${$addonAssetsUrl}slice16.png 3x`"
								/>
							</div>
							<div class="item-inner">
								<input
									...
									type="text"
									:placeholder="
										$t('tasks.task.add_items', 'Add items')
									"
								/>
							</div>
						</li>
     ...
     -->
					<f7-list media-list class="list-custom activity no-chevron">
						<f7-list-item
							divider
							:title="$t('tasks.task.activity', 'Activity')"
						>
						</f7-list-item>
						<f7-list-item
...
export default {
	...
	beforeUnmount() {
		const self = this;
		if (self.datePicker && self.datePicker.destroy)
			self.datePicker.destroy();
		if (self.statusPicker && self.statusPicker.destroy)
			self.statusPicker.destroy();
	...
	methods: {
		...
		addChecklistItem(input) {
			...
			if (!task.data.checklist.items)
				self.$set(task.data.checklist, "items", []);
		...
		toggleChecklistItem(item) {
			...
			self.$set(
				self.task.data.checklist,
				"items",
				self.task.data.checklist.items
			);
			self.saveTask();
		},
		removeChecklistItem(item) {
			const self = this;
			self.task.data.checklist.items.splice(
				self.task.data.checklist.items.indexOf(item),
				1
			);
			self.$set(
				self.task.data.checklist,
			...
			self.statusPicker = self.$f7.picker.create({
				...
				cols: [
					{
						textAlign: "center",
						values: taskStatuses,
						displayValues: taskStatuses.map((s) =>
							self.$t(
								`tasks.status.${s
									.toLowerCase()
									.replace(/ /g, "_")}`,
				...
				on: {
					close(picker) {
						...
						self.addActivity(
							"status",
							self.$t("tasks.task.changed_status_to", {
