<template>
	<f7-page
		class="tasks__page"
		name="tasks__date-range-select"
		id="tasks__date-range-select"
	>
		<f7-navbar>
			<tommy-nav-back></tommy-nav-back>
			<f7-nav-title>
				{{ $t("tasks.common.date_range", "Date Range") }}
			</f7-nav-title>
			<f7-nav-right>
				<f7-link v-if="showSave" @click="save" icon-f7="check">
				</f7-link>
			</f7-nav-right>
		</f7-navbar>
		<f7-list>
			...
			<f7-list-item
				...
				:title="$t('tasks.date_range.today', 'Today')"
			>
			</f7-list-item>
			<f7-list-item
				radio
				...
				:title="$t('tasks.date_range.week', 'This Week')"
			>
			</f7-list-item>
			<f7-list-item
				radio
				...
				:title="$t('tasks.date_range.month', 'This Month')"
			>
			</f7-list-item>
		</f7-list>
		<f7-list class="top-10 list-custom">
			...
			>
				<template #after>
					<f7-toggle
						...
						@change="toggleCustomRange($event.target.checked)"
					>
					</f7-toggle>
				</template>
			</f7-list-item>
			<f7-list-item
				v-show="isCustomRange"
				:label="$t('tasks.common.start_date', 'Start Date')"
				type="text"
				ref="rangeStart"
			>
			</f7-list-item>
			<f7-list-item
				v-show="isCustomRange"
				:label="$t('tasks.common.end_date', 'End Date')"
				type="text"
				ref="rangeEnd"
			>
			</f7-list-item>
...
export default {
	...
	data() {
		...
		return {
			showSave: false,
			dateFrom:
				Array.isArray(range) && range[0] ? range : new Date().getTime(),
			dateTo:
				Array.isArray(range) && range[1] ? range : new Date().getTime(),
			range,
		};
	},
	computed: {
		isCustomRange() {
			...
			return (
				Array.isArray(range) || !(!range || typeof range === "string")
			);
		},
	},
	mounted() {
		...
		self.calendarFrom = self.$f7.calendar.create({
			...
			on: {
				change(calendar, values) {
					...
					if (Array.isArray(self.range) && self.range[0])
						self.range[0] = self.dateFrom;
		...
		self.calendarTo = self.$f7.calendar.create({
			...
			on: {
				change(calendar, values) {
					...
					if (Array.isArray(self.range) && self.range[1])
						self.range[1] = self.dateTo;
