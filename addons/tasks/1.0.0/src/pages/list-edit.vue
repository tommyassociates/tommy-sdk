<template>
	<f7-page name="tasks__list-edit" id="tasks__list-edit" class="tasks__page">
		<f7-navbar>
			...
			<f7-nav-title>
				{{ $t("tasks.list-edit.title", "Edit List") }}
			</f7-nav-title>
			<f7-nav-right>
		...
		<f7-list v-if="list" class="list-custom">
			<f7-list-item
				:label="$t('tasks.list-edit.name', 'Name')"
				type="text"
				:value="list.name"
			...
			<f7-list-item
				:title="
					$t(
						'tasks.list-edit.show-fast-add-task',
						'Show Fast Add Task'
					)
				"
			>
				<template #after>
					<f7-toggle
						:checked="list.data.show_fast_add"
						@change="toggleFastAdd($event.target.checked)"
					>
					</f7-toggle>
				</template>
			</f7-list-item>
		...
		<f7-list v-if="list && !list.data.default" class="margin-top">
			<f7-list-button
				...
				@click="deleteList"
			>
				{{ $t("tasks.list-edit.delete-list", "Delete List") }}
			</f7-list-button>
		</f7-list>
	</f7-page>
</template>
...
export default {
	...
	mounted() {
		...
		API.loadList(self.id).then((list) => {
			...
			self.list = list;
			self.$api
				.getInstalledAddonPermission("tasks", "task_list_read_access", {
					taggable_id: list.id,
					with_filters: true,
				})
				.then((permission) => {
			...
			self.$api
				.getInstalledAddonPermission("tasks", "task_list_edit_access", {
					taggable_id: list.id,
					with_filters: true,
				})
				.then((permission) => {
	...
	methods: {
		...
		showDateRange() {
			...
			self.f7router.navigate(
				`/tasks/list-edit/${self.list.id}/date-range/`,
				{
					props: {
						list: self.list,
					},
