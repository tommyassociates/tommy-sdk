<template>
	<div class="tag-select tasks__tag-select tasks__participats-select">
		<f7-list media-list class="list-custom participants top-0">
			<f7-list-item
				:title="$t('tasks.task.participants', 'Participants')"
				divider
			>
			</f7-list-item>
			<li class="item-content">
				<div class="item-inner">
					<template v-for="(tag, index) in tags">
						<img
							v-if="tag.context === 'members'"
							:key="index"
							:src="teamMemberIconUrl(tag.user_id)"
							class="circle item"
						/>
						<div v-else :key="index" class="chip item">
							<div class="chip-media">
								<img
									:src="contextIconSrc(tag)"
									:srcset="contextIconSrcset(tag)"
								/>
							</div>
					...
					<a
						href="#"
						class="item icon-only tag-search"
						@click="openSelector"
					>
						<img
							:src="`${$addonAssetsUrl}slice6.png`"
							:srcset="`${$addonAssetsUrl}slice6@2x.png 2x,${$addonAssetsUrl}slice6.png 3x`"
						/>
					</a>
				</div>
			</li>
		...
		<f7-popover
			id="task-participants-popover"
			target=".tag-select .item.tag-search"
			v-if="teamTags"
			:opened="popoverOpened"
			@popover:closed="popoverOpened = false"
		>
			<f7-list class="no-margin tasks__tag-select-tags-list" no-hairlines>
				...
				>
					<template #media v-if="tag.context === 'members'">
						<span
							class="tag-select-list-avatar"
							:style="`background-image: url(${tag.icon || ''})`"
						>
						</span>
					</template>
					<template #media v-else>
						<span class="tag-select-list-icon">
							<img
								:src="contextIconSrc(tag)"
								:srcset="contextIconSrcset(tag)"
							/>
		...
		>
			...
			<div class="page-content">
				...
				<f7-list
					class="no-margin tasks__tag-select-tags-list"
					no-hairlines
				>
					<f7-list-item
						v-for="(tag, index) in teamTags"
