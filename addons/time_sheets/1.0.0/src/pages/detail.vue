<template>
  ...
  >
    ...
    <template v-if="loaded">
      ...
      <template v-if="timesheetItems.length">
        <f7-list media-list>
          <f7-list-item
            v-for="(timesheetItem, index) in timesheetItems"
            :key="'timesheetItem_'+index"
            :title="timesheetItem.title"
            :link="'/time-sheets/detail/' + timesheetItem.id"
          >
            <div
              slot="media"
            >
              <div class="circle">
                <div class="circle__content">
                  <span class="circle__text">{{ timesheetItem.hours }}h</span>
                  <span class="circle__text circle__text--small">{{ timesheetItem.minutes }}m</span>
                </div>
              </div>
            </div>

            <div class="description">{{ timesheetItem.description }} desc</div>
          </f7-list-item>
        </f7-list>
      </template>
      <template v-else>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      hoursTotal() {
        const self = this;
        const items = self.timesheetItems;
        const hours = items.reduce((totalHours, item) => +totalHours + +item.work_hours, 0);
      ...
      timesheetItems() {
        const self = this;
        const timesheetItems = self.timesheetsItemsData.filter(timesheetItem => +timesheetItem.timesheet_id === +self.edit_id);
        return TimesheetService.formatTimesheetsItemsData(timesheetItems, self);
      },
    },
    mounted() {
      const self = this;
