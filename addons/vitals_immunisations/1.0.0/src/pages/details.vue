<template>
  <f7-page id="vitals_element__add" :class="`vitals-element-manual-add-page vitals-${vitalsElement}-manual-add-page vitals-${vitalsElement}-details-page`">
    ...
    <div class="immunisations-details-buttons" v-if="archived">
      <a href="#" class="immunisations-details-button-red" @click="deleteVaccine">{{$t('vitals_immunisations.archive.delete_button')}}</a>
    </div>
  </f7-page>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      tAdd(v, d) {
        return this.$t(`${this.addon}.add_vaccine.${v}`, d);
      },
      tDetails(v, d) {
        return this.$t(`${this.addon}.immunisation_details.${v}`, d);
      },
      statusText() {
        const self = this;
      ...
      archiveVaccine() {
        ...
        self.$f7.dialog.confirm(self.$t('vitals_immunisations.archive_delete_confirm_prompt.text'), () => {
      ...
      injectVaccine() {
        ...
        const dialog = self.$f7.dialog.create({
          title: self.$t('vitals_immunisations.injection_confirm.date_label'),
          text: '<div class="immunisations-dialog-calendar"></div>',
          buttons: [
            {
              text: self.$t('vitals_immunisations.injection_confirm.cancel_button'),
            },
            {
              text: self.$t('vitals_immunisations.injection_confirm.confirm_button'),
              bold: true,
              onClick() {
                inject();
              },
