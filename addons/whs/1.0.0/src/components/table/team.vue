<template>
  <div class="whs-table-container">
    <template v-if="loaded">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in data" :key="'item_'+index">
              <td class="media-cell media-cell-image" :style="[item.icon_url ? {'background-image': `url(${item.icon_url})`}: teamStyle]" ></td>
      ...
      <empty-block v-if="data.length === 0" :text="$t('whs.common.no', {text: $t('whs.summary_page.roles')})" />
    </template>
    <template v-if="!loaded">
...
export default {
  ...
  methods: {
    loadData(page = 1) {
      ...
      ///test pagination
      self.$events.$emit("pagination:" + self.parent + ":team:set", {
        total: 12,
        rows: 39
      });
    },
  ...
  created() {
    const self = this;
    self.$events.$on(
      "pagination:" + self.parent + ":team:change",
