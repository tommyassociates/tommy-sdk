    ...
    <f7-navbar>
      ...
      <f7-subnavbar class="main-subnavbar">
        ...
        <div class="whs-menubar" v-if="searchEnabled">
          ...
          <a
            :class="`link ${activeSearchFilter === 'items' ? 'whs-menubar-active' : ''}`"
          ...
          <a
            :class="`link ${activeSearchFilter === 'locations' ? 'whs-menubar-active' : ''}`"
          ...
          <a
            :class="`link ${activeSearchFilter === 'tags' ? 'whs-menubar-active' : ''}`"
    ...
    <template v-if="activeTab === 'activity' && !searchEnabled">
      ...
      <f7-list class="whs-list whs-main-activity" media-list>
        <f7-list-item divider class="filter">
          ...
          <f7-button fill>{{$t('whs.common.dashboard')}}</f7-button>
        </f7-list-item>
      </f7-list>
      <f7-list class="whs-list whs-main-activity all" media-list v-if="activity_filter === 'all'">
        <f7-list-item divider class="more">
          <f7-link @click="actionMoreClick('item')">{{$t('whs.common.more')}}</f7-link>
        </f7-list-item>
        <f7-list-item divider class="divider-title">{{settings.item.plural_name}}</f7-list-item>
        <f7-list-item-row>
          <main-list
            type="item"
        ...
        <f7-list-item divider class="more">
          <f7-link @click="actionMoreClick('location')">{{$t('whs.common.more')}}</f7-link>
        </f7-list-item>
        <f7-list-item divider class="divider-title">{{settings.location.plural_name}}</f7-list-item>
        <f7-list-item-row>
          <main-list
            type="location"
        ...
        <f7-list-item divider class="more">
          <f7-link @click="actionMoreClick('tag')">{{$t('whs.common.more')}}</f7-link>
        </f7-list-item>
        <f7-list-item divider class="divider-title">{{settings.tag.plural_name}}</f7-list-item>
        <f7-list-item-row>
          <main-list
            type="tag"
        ...
        <f7-list-item divider class="more">
          <f7-link @click="actionMoreClick('role')">{{$t('whs.common.more')}}</f7-link>
        </f7-list-item>
        <f7-list-item divider class="divider-title">{{$t('whs.popover.activity.role')}}</f7-list-item>
        <f7-list-item-row>
          <main-list
            type="role"
        ...
        <f7-list-item divider class="more">
          <f7-link @click="actionMoreClick('team')">{{$t('whs.common.more')}}</f7-link>
        </f7-list-item>
        <f7-list-item divider class="divider-title">{{$t('whs.popover.activity.team')}}</f7-list-item>
        <f7-list-item-row>
          <main-list
            type="team"
      ...
      <f7-list v-else class="selected whs-list whs-main-activity" media-list>
        ...
        >
          <div
            ...
            :style="[target.image ? {'background-image': `url(${target.image})`}: dynamicStyle]"
          ></div>
...
*/
      ...
      API.getActivities().then(data => {
        console.log("TCL: getActivities -> data", data);
      });

      //witot limit page
      API.getItem(options).then(data => {
        self.activities_main.items = [data[0], data[1]];
        self.loaded.activity.item = true;
      });
      API.getLocations(options).then(data => {
        self.activities_main.locations = [data[0], data[1]];
        self.loaded.activity.location = true;
      });
      API.getTags(options).then(data => {
        self.activities_main.tags = [data[0], data[1]];
        self.loaded.activity.tag = true;
      });
      API.getRoles(options).then(data => {
        self.activities_main.roles = [data[0], data[1]];
        self.loaded.activity.role = true;
      });
      API.getTeam(options).then(data => {
        self.activities_main.team = [data[0], data[1]];
        self.loaded.activity.team = true;
      });
    },
    getActivitiesMore(target) {
      ...
      switch (target) {
        case "item":
          API.getItem(options).then(data => {
            self.activities_more.data = data;
            self.loaded.activity_more = true;
          });
          break;
        case "location":
          API.getLocations(options).then(data => {
        ...
        if (new_settings[def] === undefined) {
          ...
          for (itemDef in def_settings[def]) {
            if (new_settings[def][itemDef] === undefined) {
              ...
              //deper check
              new_settings[def][itemDef] = self.compareDefSettings(
                def_settings[def][itemDef],
                new_settings[def][itemDef]
              );
            }
          }
        }
