...
let ColorPicker;
function createColorPicker(f7app, colorPickerParams = {}) {
  if (ColorPicker) {
    return new ColorPicker(f7app, colorPickerParams);
  }
  ColorPicker = class extends (f7app.constructor.Class) {
    constructor(app, params = {}) {
      super(params, [app]);
      const self = this;

      self.params = app.utils.extend({}, defaults, params);

      let $containerEl;
      if (self.params.containerEl) {
        $containerEl = app.$(self.params.containerEl);
        if ($containerEl.length === 0) return self;
      }

      let $inputEl;
      if (self.params.inputEl) {
        $inputEl = app.$(self.params.inputEl);
      }

      let $targetEl;
      if (self.params.targetEl) {
        $targetEl = app.$(self.params.targetEl);
      }
      ...
      function onHtmlClick(e) {
        ...
        const $clickTargetEl = app.$(e.target);
        ...
        } else if (app.$(e.target).closest('.sheet-modal').length === 0) {
          self.close();
        }
      }
    ...
    setValue(value = {}, updateModules = true) {
      ...
      if (value.rgb || value.rgba) {
        ...
        rgb = [r, g, b];
        hex = colorUtils.colorRgbToHex(...rgb);
        hsl = colorUtils.colorRgbToHsl(...rgb);
        hsb = colorUtils.colorHslToHsb(...hsl);
      ...
      if (value.hsl || value.hsla) {
        ...
        hsl = [h, s, l];
        rgb = colorUtils.colorHslToRgb(...hsl);
        hex = colorUtils.colorRgbToHex(...rgb);
        hsb = colorUtils.colorHslToHsb(...hsl);
        hsl = self.normalizeHsValues(hsl);
        hsb = self.normalizeHsValues(hsb);
      ...
      if (value.hsb) {
        ...
        hsb = [h, s, b];
        hsl = colorUtils.colorHsbToHsl(...hsb);
        rgb = colorUtils.colorHslToRgb(...hsl);
        hex = colorUtils.colorRgbToHex(...rgb);
        hsl = self.normalizeHsValues(hsl);
        hsb = self.normalizeHsValues(hsb);
      ...
      if (value.hex) {
        rgb = colorUtils.colorHexToRgb(value.hex);
        hex = colorUtils.colorRgbToHex(...rgb);
        hsl = colorUtils.colorRgbToHsl(...rgb);
        hsb = colorUtils.colorHslToHsb(...hsl);
        hsl = self.normalizeHsValues(hsl);
      ...
      if (!self.initialValue) self.initialValue = app.utils.extend({}, self.value);
    ...
    onOpen() {
      ...
      if (!initialized) {
        ...
        self.initialValue = app.utils.extend({}, value);
    ...
    destroy() {
      ...
      self.app.utils.deleteProps(self);
      self.destroyed = true;
    }
  };
  return new ColorPicker(f7app, colorPickerParams);
