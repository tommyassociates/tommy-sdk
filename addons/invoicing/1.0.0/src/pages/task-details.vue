<template>
  <f7-popup @popup:close="onPopupClose">
    <f7-view :init="false">
      <f7-page id="invoicing__task" name="invoicing__task" class="invoicing-page">
        ...
        <f7-popover class="task-menu-popover">
          <f7-list>
            <f7-list-button popover-close @click="addChecklist">{{$t('invoicing.task.checklist', 'Checklist')}}</f7-list-button>
            <f7-list-button popover-close @click="addDeadline">{{$t('invoicing.task.deadline', 'Deadline')}}</f7-list-button>
          </f7-list>
        </f7-popover>
        <f7-messagebar
          :placeholder="$t('invoicing.task.enter_comment', 'Enter comment')"
          :value="comment"
          @input="comment = $event.target.value"
        >
          <f7-link slot="send-link" @click="addComment" :class="{disabled: comment.trim().length === 0}">{{$t('invoicing.task.send', 'Send')}}</f7-link>
        </f7-messagebar>
        ...
        <template v-if="task">
          <f7-block class="subheader">
            ...
            <input ref="statusInput" type="text" :value="task.status === 'Unassigned' ? $t('invoicing.task.waiting_for_assignments', 'Waiting for assignments') : taskStatus(task.status)" readonly="readonly" class="unstyled task-status-picker" />
          </f7-block>
          ...
          <f7-list media-list class="list-custom details bottom-0">
            <f7-list-item>
              ...
              <f7-input
                type="textarea"
                class="inline-edit"
                :placeholder="$t('invoicing.task.edit_description', 'Edit the description...')"
                clearButton
          ...
          <f7-list media-list class="list-custom details top-0 bottom-0" v-show="showDeadline || task.end_at">
            <f7-list-item>
              ...
              <div class="item-input-wrap" slot="inner">
                <input ref="deadlineInput" type="text" class="unstyled edit-task-deadline" :placeholder="$t('invoicing.task.edit_deadline', 'Edit deadline...')">
              </div>
          ...
          <f7-list media-list class="list-custom checklist top-0" v-if="showChecklist || (task.data.checklist && task.data.checklist.items)">
            <li class="item-content">
              ...
              <div class="item-inner">
                <div>{{$t('invoicing.task.checklist', 'Checklist')}}</div>
            ...
            <li class="item-content">
              ...
              <div class="item-inner">
                <input style="height: auto" @keypress.enter="addChecklistItem($event.target)" type="text" :placeholder="$t('invoicing.task.add_items', 'Add items')">
              </div>
            </li>
          ...
          <f7-list media-list class="list-custom activity no-chevron">
            <f7-list-item divider :title="$t('invoicing.task.activity', 'Activity')"></f7-list-item>
            <f7-list-item
  ...
  export default {
    ...
    methods: {
      ...
      onPopupClose() {
        const self = this;
        if (self.taskChanged) {
          self.$events.$emit('invoicing:reloadListsTasks');
        }
      },
      saveTaskName() {
        const self = this;
          ...
          cols: [
            {
              textAlign: 'center',
              values: taskStatuses,
              displayValues: taskStatuses.map(s => self.$t(`invoicing.status.${s.toLowerCase().replace(/ /g, '_')}`, { defaultValue: s })),
            },
          ],
          on: {
            close(picker) {
              ...
              task.status = status;
              self.addActivity('status', self.$t('invoicing.task.changed_status_to', { status: translatedStatus }));
            },
          },
        });
      },
