      ...
      <f7-page name="invoicing__order-details" id="invoicing__order-details" class="invoicing-page">
        ...
        <template v-if="order">
          <f7-list class="list-custom">
            <f7-list-item v-if="order.created_at && order.id" :title="$t('invoicing.order_details.created')"
              :after="formatDate(order.created_at)"></f7-list-item>

            <f7-list-item :title="$t('invoicing.order_details.due_date')" :link="true"
              :after="formatOrderDate(order.data ? parseInt(order.data.date, 10) : null)" @click="openDateSelect">
            </f7-list-item>
            <f7-list-item :title="$t('invoicing.order_details.due_time')" :link="true"
              :after="formatOrderTime(order.data ? parseInt(order.data.date, 10) : null)" @click="openTimeSelect">
            </f7-list-item>

            <f7-list-input v-if="order.data && order.data.location" :label="$t('invoicing.order_details.city')"
              :placeholder="$t('invoicing.order_details.city_placeholder')" type="text"
              :value="order.data.location.city" @input="onCityChange"></f7-list-input>

            <f7-list-input v-if="order.data && order.data.location" :label="$t('invoicing.order_details.address')"
              :placeholder="$t('invoicing.order_details.address_placeholder')" type="text"
              :value="order.data.location.address" @input="onAddressChange"></f7-list-input>

            <f7-list-item :title="$t('invoicing.order_details.status')" smart-select
              :smart-select-params="{openIn: 'popover', closeOnSelect: true, routableModals: false}">
              <select @change="onStatusChange">
                <option v-for="(status, index) in orderStatuses" :key="index" :value="status"
                  :selected="order.status === status">{{$t(`invoicing.order_status.${status}`)}}</option>
              </select>
            </f7-list-item>
            <f7-list-item :title="$t('invoicing.order_details.type')" smart-select
              :smart-select-params="{openIn: 'popover', closeOnSelect: true, routableModals: false}">
            ...
            <f7-list-input type="textarea" :value="order.comment"
              :placeholder="$t('invoicing.order_details.comment_placeholder')" @input="onCommentChange" resizable>
            </f7-list-input>
            ...
            <template v-if="orderAssignee">
              ...
              <f7-list-item :title="orderAssignee ? orderAssigneeName : ''" smart-select :smart-select-params="{virtualList: true}">
                ...
                <f7-link slot="root" style="margin-left: 80px; padding: 8px 0"
                  :href="`/team-member-details/?user_id=${order.assignee_id}&name=${orderAssigneeName}`">
            ...
            <template>
              ...
              <f7-list-item :title="orderUser ? orderUserName : ''" smart-select  :smart-select-params="{virtualList: true}">
                ...
                <f7-link slot="root" style="margin-left: 80px; padding: 8px 0"
                  :href="`/contact-details/?user_id=${order.assignee_id}`">
            ...
            <template v-if="order && order.data.feedback && products && packages">
              ...
              <f7-list-item :title="$t('invoicing.order_feedback_results.scheduled')" :after="calcOrderDuration()" />
              <f7-list-item :title="$t('invoicing.order_feedback_results.start')"
                :after="$moment(parseInt(order.data.date, 10)).format('H:mm DD MMM YYYY')" />
              <f7-list-item :title="$t('invoicing.order_feedback_results.finish')"
              ...
              <f7-list-item :title="$t('invoicing.order_feedback_results.question_1')"
                :after="$t(`invoicing.order_feedback_results.${order.data.feedback.question1}`)" />
              <f7-list-item :title="$t('invoicing.order_feedback_results.question_2')"
                :after="$t(`invoicing.order_feedback_results.${order.data.feedback.question2}`)" />
              <f7-list-item :title="$t('invoicing.order_feedback_results.question_3')"
          ...
          <f7-list v-if="orderCancelable" class="margin-top margin-bottom">
            <f7-list-button color="custom" class="color-custom" @click="cancelOrder">
              {{$t('invoicing.order_details.cancel_button', 'Cancel Order')}}</f7-list-button>
          </f7-list>
...
export default {
  ...
  mounted() {
    ...
    if (self.id) {
      API.loadOrder(self.id).then((order) => {
        self.order = order;
        let orderUser = self.$root.teamMembers.filter(
          m => m.user_id === parseInt(order.user_id, 10)
        )[0];
        const orderAssignee = self.$root.teamMembers.filter(
          m => m.user_id === parseInt(order.assignee_id, 10)
        )[0];
        ...
        if (!orderUser) {
          if (self.contacts) {
            ...
            orderUser = self.contacts.filter(
              c => c.friend_id === parseInt(order.user_id, 10)
            )[0];
  ...
  methods: {
    ...
    onAssigneeChange(e) {
      ...
      const assignee = self.$root.teamMembers.filter(
    ...
    openTimeSelect() {
      ...
      const initialValue = self.formatOrderTime(
