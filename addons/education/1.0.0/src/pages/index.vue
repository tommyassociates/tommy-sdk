    ...
    <template v-if="fragment">
      ...
      <f7-list class="no-margin-top no-hairline-top">
        ...
        <f7-list-item
          ...
          :after="`${currentCourse.duration} ${t('hours_label')}`"
        />
        <f7-list-item
          :title="t('outcome_label')"
          :after="currentCourse.outcome"
        />
        <f7-list-item
          :title="t('estimate_data_label')"
          :after="`${currentCourse.estimate_data} ${t('mb_label')}`"
        />
...
<script>
  ...
  export default {
    data() {
      const actorId = this.$f7route.query.actor_id ? parseInt(this.$f7route.query.actor_id, 10) : undefined;
      API.actorId = actorId;
      return {
        courses,
    ...
    computed: {
      ...
      requiredCourses() {
        return this.courses.filter(c => this.currentCourse.required.indexOf(c.id) >= 0);
      },
    },
    mounted() {
      this.getData();
